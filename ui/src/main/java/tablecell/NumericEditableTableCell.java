...
public class NumericEditableTableCell<S extends Object, T extends Number> extends AbstractEditableTableCell<S, T> {
    ...
    public NumericEditableTableCell() {
        this( NumberFormat.getInstance(), true);
    }
    ...
    protected void commitHelper( boolean losingFocus ) {
        if( textField == null || losingFocus) {
            return;
        }

        try {
            String input = textField.getText();
            if (this.getTableView().getEditingCell().getRow() >= this.getTableView().getItems().size()) return;
            Number original = (Number) ((Map.Entry) this.getTableView().getItems().get(this.getTableView().getEditingCell().getRow())).getValue();
            ...
            Number number;
            if (FormatChecker.isInteger(input)) {
            ...
            if (original instanceof Integer) {
                if (number instanceof Integer) {
                    ...
                    AlertManager.warning("Invalid value.", "Please enter an integer value.");
                }
            ...
            AlertManager.warning("Invalid value.", "Please enter a numerical value.");
        }
    }
}